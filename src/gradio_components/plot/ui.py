...
def build_plot_section() -> dict[str, Any]:
    ...
    with gr.Row():
        ...
        plot_max_points_input = gr.Slider(minimum=1, maximum=12000, step=1, value=4000, label="Max plotted points")
        plot_animate_input = gr.Checkbox(label="Animate over time (t0 → tn)", value=False)
    with gr.Row():
